...
def compute_metrics_with_threshold(all_scores: List[List[float]], all_labels: List[List[int]],threshold: float, invert: bool,) -> Dict[str, Any]:
    """
    이미 정해진 threshold / invert로 metric만 계산.
    (global threshold를 카테고리 subset에도 그대로 적용할 때 사용)
    """
    assert len(all_scores) == len(all_labels), "scores/labels length mismatch"
    scores_flat = np.concatenate([np.asarray(s, dtype=np.float32) for s in all_scores], axis=0)
    labels_flat = np.concatenate([np.asarray(l, dtype=np.int32) for l in all_labels], axis=0)
    ...
    f1_neg = _f1_for_error(labels_flat, pred)
    f1_pos = _f1_for_correct(labels_flat, pred)
    prm_score = 0.5 * (f1_neg + f1_pos)
    acc = _accuracy(labels_flat, pred)
    pos_acc, neg_acc = _positive_negative_acc(labels_flat, pred)
...
def collect_scores_and_labels(ds, prm: ProcessRewardModel, tokenizer,  batch_size: int = 12, rw_token: str = "<RW>", max_length: Optional[int] = None,
    apply_sigmoid: bool = False, category_field: Optional[str] = None,) -> Tuple[List[List[float]], List[List[int]], List[Optional[str]], List[Any]]:
    """
    전체 dataset에 대해 딱 한 번 PRM을 돌려서
    - all_scores, all_labels (per example)
    - all_categories (per example category 값)
    - all_ids (optional: ex["idx"]가 있으면 같이 저장)
    을 모두 수집.
    """
    ...
    has_cat = category_field is not None and category_field in ds.column_names
    has_idx = "idx" in ds.column_names
    ...
    for i in tqdm(range(0, N, batch_size), desc="Scoring PRMBench", dynamic_ncols=True):
        ...
        # meta
        if has_cat:
            cats_batch = list(sub[category_field])
        else:
            cats_batch = [None] * len(sub)
        if has_idx:
            idx_batch = list(sub["idx"])
        else:
            idx_batch = [None] * len(sub)

        step_scores_batch = score_batch_steps(
            prm,
            tokenizer,
            questions,
            steps_list,
        ...
        assert len(step_scores_batch) == len(labels_list) == len(cats_batch) == len(idx_batch)
        for j, (scores, labels, cat, ex_id) in enumerate(
            zip(step_scores_batch, labels_list, cats_batch, idx_batch)
        ):
    ...
    return all_scores, all_labels, all_categories, all_ids

# ------------------------- Category-wise eval ------------------------- #
def evaluate_split(all_scores: List[List[float]], all_labels: List[List[int]], all_categories: Optional[List[Optional[str]]] = None,) -> Tuple[Dict[str, Any], Dict[str, Dict[str, Any]]]:
    """
    PRM forward는 이미 끝났다고 가정하고,
    - 전체 threshold 튜닝 (글로벌)
    - 같은 threshold로 카테고리별 metric 계산
    만 수행.
    """
    print("==== [OVERALL] ====")
    ...
    # unique categories (None 제외)
    cats = sorted({c for c in all_categories if c is not None})
    if not cats:
        return best_overall, category_results

    print(f"\n[INFO] Category values detected: {cats}")
    thr = best_overall["threshold"]
    invert = best_overall["invert"]

    for cat in cats:
        # 해당 category에 속한 example index들
        idxs = [i for i, c in enumerate(all_categories) if c == cat]
        if not idxs:
            continue
        scores_cat = [all_scores[i] for i in idxs]
        labels_cat = [all_labels[i] for i in idxs]

        metrics_cat = compute_metrics_with_threshold(scores_cat, labels_cat, thr, invert)

        print(f"\n==== [CATEGORY={cat}] ====")
        print(
            f"[METRIC][{cat}] "
            f"Neg-F1={metrics_cat['f1_neg']*100:.2f}, "
            f"Pos-F1={metrics_cat['f1_pos']*100:.2f}, "
            f"PRMScore={metrics_cat['prm_score']*100:.2f}, "
            f"Acc={metrics_cat['acc']*100:.2f}, "
            f"Pos-Acc={metrics_cat['pos_acc']*100:.2f}, "
            f"Neg-Acc={metrics_cat['neg_acc']*100:.2f}"
        )

        category_results[cat] = metrics_cat
...
def parse_args():
    ...
    p.add_argument("--out_path", type=str, default="/home/leena/rs_prm/analysis/prmbench")
    return p.parse_args()
...
def main():
    ...
    # 1) PRM forward는 여기서 딱 한 번만 수행
    all_scores, all_labels, all_categories, all_ids = collect_scores_and_labels(
        ds, prm, tokenizer, batch_size=args.batch_size,
        rw_token=rw_tok, max_length=args.max_length,
        apply_sigmoid=args.apply_sigmoid, category_field=args.category_field,)

    # 2) 저장된 점수/라벨로만 전체 + 카테고리별 metric 계산
    best_overall, category_results = evaluate_split( all_scores, all_labels, all_categories=all_categories)
    ...
    # per-example raw 결과도 같이 저장해서 나중에 offline 분석 가능하게
    per_example = []
    for scores, labels, cat, ex_id in zip(all_scores, all_labels, all_categories, all_ids):
        per_example.append(
            {
                "idx": ex_id,
                "category": cat,
