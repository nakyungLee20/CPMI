...
def generate_reward_distribution_plots( *, step_scores_list: List[List[float]], gold_first_err: List[int], pred_first_err: List[int], out_dir: str, split_name: str, label_by: str = "gold",  metrics_sel: List[str] = None, bins: int = 60, norm_mode: str = "density",  title_suffix: str = "") -> None:
    os.makedirs(out_dir, exist_ok=True)
    ...
    for mode in label_modes:
        ...
        try:
            ...
            for m in metric_names:
                ...
                _ = plot_normed_hist_by_correct(df_steps, m, subdir, bins=bins, normalize=norm_mode, title_postfix=(title_suffix or f"{split_name} / label_by={mode}"))
...
def eval_processbench(prm_dir: str, configs: List[str], out_dir: str, batch_size: int = 32, threshold: Optional[float] = None, use_prob: bool = False, rw_token: Optional[str] = "<RW>",
    ...
    plot_label_by: str = "gold", plot_metrics: List[str] = None, plot_bins: int = 60,  plot_norm: str = "density", plot_title_suffix: str = "",):
    os.makedirs(out_dir, exist_ok=True)
    ...
    for cfg in configs:
        ...
        if debug:
            ...
            map_seq = per_sequence_average_precision(all_step_scores_for_split, all_labels_for_split, use_prob=use_prob, invert=bool(invert_score))
            ...
            plot_root = os.path.join(out_dir, f"{cfg}_analysis")  # 동일 폴더에 하위 디렉토리 생성
            os.makedirs(plot_root, exist_ok=True)
            generate_reward_distribution_plots(
                step_scores_list=all_step_scores_for_split,
                gold_first_err=list(ds["label"]),
                pred_first_err=predictions,
                out_dir=plot_root,
                split_name=cfg,
                label_by=plot_label_by,
                metrics_sel=plot_metrics or ["logit","prob"],
                bins=plot_bins,
                norm_mode=plot_norm,
...
def main():
    ...
    ap.add_argument("--analysis_root", type=str, default="/home/leena/rs_prm/analysis/processbench/main")
    ap.add_argument("--plot_label_by", type=str, default="gold", choices=["gold","pred","both"],  help="Use gold or predicted first-error to define step correctness")
    ap.add_argument("--plot_metrics", type=str, default="logit,prob", help="Comma-separated list among: logit,prob")
    ...
    ap.add_argument("--plot_title_suffix", type=str, default="pav_cpmi")
    args = ap.parse_args()
    ...
    eval_processbench(
        ...
        plot_title_suffix=args.plot_title_suffix,
    )
