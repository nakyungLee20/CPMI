...
def _ob_ans_to_str(ans):
    ...
    if isinstance(ans, str):
        s = ans.strip()
        # 문자열이 리스트처럼 생겼으면 안전하게 파싱 시도
        if s.startswith("[") and s.endswith("]"):
            try:
                import ast
                parsed = ast.literal_eval(s)
                if isinstance(parsed, list) and parsed:
                    # 마지막/가장 신뢰 높은 값을 취할지? 보통 마지막을 많이 씁니다.
                    return _ob_ans_to_str(parsed[-1])
            except Exception:
                pass
        return s
    if isinstance(ans, list):
        # 비어있지 않은 마지막 유효 원소를 선택 (종종 마지막이 최종 정답)
        for item in reversed(ans):
            s = _ob_ans_to_str(item)
            if s:
                return s
        return ""
    if isinstance(ans, dict):
        # 흔한 키 우선순위
        for k in ["final_answer", "answer", "latex", "text", "en", "value", "content"]:
            if k in ans:
                s = _ob_ans_to_str(ans[k])
                if s:
                    return s
        # multiple 선택지
        if "multiple" in ans and isinstance(ans["multiple"], list) and ans["multiple"]:
            return _ob_ans_to_str(ans["multiple"][-1])
        return ""

    return str(ans).strip()

def _ob_pick_gold_from_solution(sol):
    # 솔루션 텍스트에서 \boxed{}나 $...$를 추출하는 fallback
    import re
    if not isinstance(sol, str):
        return ""
    s = sol.strip()
    # 1) \boxed{...} 전부 → 마지막 것을 채택
    ...
    if lines:
        dollars = re.findall(r"\$([^$]+)\$", lines[-1])
        if dollars:
            return dollars[-1].strip()
        return lines[-1]  # 정말 없으면 마지막 줄
    return ""
...
def get_loader(ds_name: str, split: str, batch_size: int):
    ...
    def _std(ex):
        ...
        if ds_name == "ob":
            out["answer"] = _ob_ans_to_str(ex.get("final_answer"))
            if not out["answer"]:
                sol = ex.get("solution") or ex.get("solutions") or ""
                if isinstance(sol, list) and sol:
                    sol = sol[-1]
                out["answer"] = _ob_pick_gold_from_solution(sol)
        return out
...
STEP_NUM_RE = re.compile(r"^\s*step\s*(\d+)\s*:", re.IGNORECASE)
...
def pick_best_by_prm(prm_model,prm_tokenizer,question: str, candidates: List[str], agg: str = "mean", rw_token: str = "<RW>", ds_name: str = "", model_type: str ="", choices: Optional[List[str]] = None,) -> Tuple[int, List[List[float]], List[float]]:
    step_scores = score_candidates_with_prm(prm_model=prm_model, prm_tokenizer=prm_tokenizer, question=question, candidates=candidates, rw_token=rw_token, ds_name=ds_name, model_type=model_type, choices=choices,)
...
##############################################################################
# PRM-guided stepwise decoding #
##############################################################################
def _dbg(msg: str, maxlen: int = 1000, enabled: bool = False) -> None:
    if enabled:
        s = msg if len(msg) <= maxlen else (msg[:maxlen] + " ...")
        print(s, flush=True)

def _format_steps_for_assistant(steps: List[str]) -> str:
    return "".join(f"Step {i+1}: {s}\n\n" for i, s in enumerate(steps))

def _build_next_step_prompt(tokenizer, question: str, steps_so_far: List[str], *, ds_name: str = "") -> str:
    k = len(steps_so_far) + 1

    if ds_name != "mmlu":
        # build_prompt는 add_generation_prompt=True 이므로 결과 끝이 assistant 차례
        base = build_prompt(question, tokenizer, dataset_name=ds_name)  # str 반환
        return base + _format_steps_for_assistant(steps_so_far) + f"Step {k}: "
    else:
        sys = (
            "You are a careful math tutor. Generate the next step ONLY.\n"
            f"Write exactly one line starting with 'Step {k}:'. Do not give the final answer now."
        )
        context_steps = "\n".join([f"Step {i+1}: {s}" for i, s in enumerate(steps_so_far)]) if steps_so_far else "(none)"
        user = f"Question:\n{question}\n\nCurrent steps:\n{context_steps}\n\nContinue with exactly one new line."
        try:
...
_STEP_TRIM_RX = re.compile(r"^\s*Step\s*\d+\s*:\s*", re.IGNORECASE)
def _clean_next_step_text(raw: str, *, k: int) -> str:
    if not raw:
        return ""
    line = raw.splitlines()[0].strip()
    line = _STEP_TRIM_RX.sub("", line).strip()
    if ANSWER_LINE_RE.search(line):
        return ""
    return line

@torch.no_grad()
def _decode_one_with_prm_vllm(item: Dict[str, Any],llm: LLM, policy_tokenizer, prm_model, prm_tokenizer, *, ds_name: str, model_type: str = "",
    n_per_step: int = 8, temperature: float = 0.6, top_p: float = 0.95, seed: int = 123, step_max_tokens: int = 512, max_steps: int = 30,
    rw_token: str = "<RW>", decode_agg: str = "last", final_answer_tokens: int = 64) -> Tuple[str, List[str], Dict[str, Any]]:
    question = item["question"]
    steps: List[str] = []
    trace = {"per_step": []}

    # stop token ids & stop strings: 한 줄 스텝만 생성하도록 제어
    im_end_id = policy_tokenizer.convert_tokens_to_ids("<|im_end|>")
    eos_id = policy_tokenizer.eos_token_id
    stop_token_ids = [tid for tid in (im_end_id, eos_id) if tid is not None]
    stop_strings = ["\nStep ", "\n\n", "\nThe answer is", "\nAnswer:"]
    ...
    for t in range(max_steps):
        # ---- (1) 다음 스텝 후보 N개 생성
        k = len(steps) + 1
        prompt = _build_next_step_prompt(policy_tokenizer, question, steps, ds_name=ds_name)
        _dbg(f"\n[decode] k={k} prompt_tail:\n{prompt[-500:]}")

        sp = SamplingParams(
            temperature=temperature, top_p=top_p, n=n_per_step, seed=seed,
            max_tokens=step_max_tokens, repetition_penalty=1.05,
            skip_special_tokens=True, stop_token_ids=stop_token_ids, stop=stop_strings
        )
        outs = llm.generate([prompt], sp, use_tqdm=False)
        cand_raw = [o.text for o in outs[0].outputs]
        cand_steps = [_clean_next_step_text(r, k=len(steps)+1) for r in cand_raw]
        cand_steps = [c for c in cand_steps if c]
        for j, c in enumerate(cand_steps):
            _dbg(f"[decode] k={k} CLEAN[{j}]: {c}")

        if not cand_steps:
            trace["per_step"].append({"k": len(steps)+1, "candidates": cand_raw, "chosen": None, "reason": "no_valid_candidates"})
            break

        # ---- (2) PRM으로 각 후보 점수 (현재까지 steps + 후보) 평가
        partial_candidates = []
        for c in cand_steps:
            new_steps = steps + [c] # "Step i: ..." 포맷으로 재결합해 PRM용 steps 배열 제작
            partial_text = "\n".join([f"Step {i+1}: {s}" for i, s in enumerate(new_steps)])
            partial_candidates.append(partial_text)

        # PRM 점수 (스텝별) → 집계 점수(마지막/평균 등)
        step_scores_list = score_candidates_with_prm( prm_model, prm_tokenizer, question, partial_candidates,
            rw_token=rw_token, ds_name=ds_name, model_type=model_type, choices=item.get("choices") if ds_name == "mmlu" else None)
        if decode_agg == "last":
            agg_scores = [(s[-1] if (s and len(s) >= 1) else float("-inf")) for s in step_scores_list]
        else:
            agg_scores = [aggregate(s, how=decode_agg) for s in step_scores_list]

        # 점수 로그
        for j, (ss, a) in enumerate(zip(step_scores_list, agg_scores)):
            _dbg(f"[decode] k={k} PRM[{j}] step_scores={['{:.3f}'.format(x) for x in ss]}  agg({decode_agg})={a:.4f}")

        # ---- (3) 최고 점수 후보 선택
        best_local = int(max(range(len(cand_steps)), key=lambda i: agg_scores[i]))
        chosen_step = cand_steps[best_local]
        
        _dbg(f"[decode] k={k} CHOSEN idx={best_local} → {chosen_step}")

        steps.append(chosen_step)
        trace["per_step"].append({
            "k": len(steps),
            "candidates": cand_steps,
            "prm_step_scores": step_scores_list,
            "prm_agg_scores": agg_scores,
        ...
        if len(steps) >= 2 and steps[-1] == steps[-2]:
            trace["per_step"][-1]["early_stop"] = "repeated_step"
            _dbg(f"[decode] k={k} early_stop: repeated_step")
            break

    # ---- (4) 최종 답 한 줄 생성
    if ds_name != "mmlu":
        base_final = build_prompt(question, policy_tokenizer, dataset_name=ds_name)
        prefix_final = _format_steps_for_assistant(steps) + "Answer: "
        final_prompt = base_final + prefix_final
        _dbg(f"[final] prompt_tail:\n{final_prompt[-500:]}")

        sp_final = SamplingParams(
            temperature=0.0, top_p=1.0, n=1, seed=seed,
            max_tokens=final_answer_tokens, repetition_penalty=1.0,
            skip_special_tokens=True, stop_token_ids=stop_token_ids,
            stop=["\n"]   # 한 줄만 출력
        )
        final_out = llm.generate([final_prompt], sp_final, use_tqdm=False)[0].outputs[0].text.strip()
        final_line = "Answer: " + final_out
        _dbg(f"[final] line: {final_line}")
    else:
        # MMLU는 letter형 포맷이 흔하므로 이전 방식 유지(원한다면 동일 패턴으로 바꿔도 됨)
        sys = "You are a precise solver. Give only the final answer."
        joined = "\n".join([f"Step {i+1}: {s}" for i, s in enumerate(steps)]) if steps else "(none)"
        ask = "Based on the steps above, output exactly one line in the format:\nThe answer is: <LETTER>\n(LETTER must be one of A, B, C, D)."
        try:
            base = policy_tokenizer.apply_chat_template(
                [{"role": "system", "content": sys},
        ...
        final_out = llm.generate([base], sp_final, use_tqdm=False)[0].outputs[0].text.strip()
        final_line = final_out
        _dbg(f"[final][mmlu] line: {final_line}")

    # 최종 본문(스텝 + 최종답 한 줄)
    final_text = (_format_steps_for_assistant(steps).rstrip() + ("\n" if steps else "") + final_line).strip()
    return final_text, steps, trace

def evaluate_with_prm_decode_vllm(dataset, llm: LLM, policy_tokenizer, prm_model, prm_tokenizer, *, ds_name: str, model_type: str = "", limit: Optional[int] = None, n: int = 8, temperature: float = 0.6,
    top_p: float = 0.95, seed: int = 123, max_tokens: int = 512, save_incorrect_path: Optional[str] = None, scorer: Any = None, decode_max_steps: int = 12, decode_agg: str = "last") -> Tuple[float, List[Dict[str, Any]], List[Dict[str, Any]]]:

    if prm_model is None or prm_tokenizer is None:
        raise ValueError("PRM-guided decoding requires a valid PRM model & tokenizer.")
    ...
    for i_ex in indices:
        ...
        final_text, steps, trace = _decode_one_with_prm_vllm(
            item, llm, policy_tokenizer, prm_model, prm_tokenizer,
            ds_name=ds_name, model_type=model_type,
            n_per_step=n, temperature=temperature, top_p=top_p, seed=seed,
            step_max_tokens=max_tokens, max_steps=decode_max_steps, decode_agg=decode_agg
        )
        ...
        if ds_name == "mmlu":
            ...
            pred_label = scorer.extract_pred(final_text, choices)
            is_correct = scorer.grade(final_text, gold_idx, choices)
            log_rec = {
                "idx": i_ex,
                "question": item["question"],
                "choices": choices,
            ...
            gold = scorer.extract_gold(item["answer"])
            pred = scorer.extract_pred(final_text)
            is_correct = scorer.grade(pred, gold)
            log_rec = {
                "idx": i_ex,
                "question": item["question"],
                "gold": gold,
        ...
        correct += int(is_correct)
        if not is_correct:
            incorrect_samples.append(log_rec)

        if (total % 20) == 0:
            acc = 100.0 * correct / max(total, 1)
            print(f"[{total}/{N}] running DECODE acc = {acc:.2f}% (N/step={n}, agg={decode_agg}, max_steps={decode_max_steps})", flush=True)

    acc = 100.0 * correct / max(total, 1)
    print(f"{ds_name} PRM-guided Decode Accuracy = {acc:.2f}% on {total} examples. (N/step={n}, agg={decode_agg}, max_steps={decode_max_steps})")
...
if __name__ == "__main__":
    ...
    ap.add_argument('--limit', type=int, default=None) # None for all
    ...
    ap.add_argument('--model_type', type=str, default='prompt_', help="Prompt style flags: '', 'prompt_*', 'contr_*'")
    ap.add_argument('--norm_type', type=str, default='norm', choices=['cal', 'norm'])
    ap.add_argument('--infer_type', type=str, default='bon', choices=['bon', 'single', 'maj', 'decode'])
    ...
    need_prm = (
        args.infer_type in {"bon", "decode"}
        or (args.infer_type == "maj" and (args.maj_use_prm_weights or args.maj_tie_break == "prm"))
    )
    prm_model = None
    prm_tokenizer = None
    prm_tag = "noPRM"
    ...
    incorr_path = os.path.join(incorr_dir, f"{args.dataset}_{args.infer_type}_{prm_tag}.json")
    ...
    if args.infer_type == 'single':
        ...
        acc, logs, incorrect = evaluate_dataset_bon_vllm(datas, policy, tokenizer, prm_model, prm_tokenizer, ds_name=args.dataset, model_type=args.model_type, model_name=model_name,
            n=args.n, temperature=args.temperature, top_p=args.top_p, limit=args.limit, seed=args.seed, max_tokens=args.max_new_tokens, batch_size=args.batch_size,
        ...
        acc, logs, incorrect = evaluate_dataset_majority_vllm(datas, policy, tokenizer, prm_model, prm_tokenizer, ds_name=args.dataset, model_name=model_name,
            ...
            seed=args.seed, max_tokens=args.max_new_tokens, batch_size=args.batch_size, agg=args.bon_agg, use_prm_weights=False,
            maj_temp=1.0, tie_break=args.maj_tie_break, save_incorrect_path=incorr_path, scorer=SCORER,
        )
    elif args.infer_type == 'decode':
        print(f"Evaluating {args.dataset} with PRM-guided decoding...", flush=True)
        acc, logs, incorrect = evaluate_with_prm_decode_vllm(datas, policy, tokenizer, prm_model, prm_tokenizer, ds_name=args.dataset, model_type=args.model_type,
            n=args.n, temperature=args.temperature, top_p=args.top_p, seed=args.seed, max_tokens=args.max_new_tokens, limit=args.limit, save_incorrect_path=incorr_path, scorer=SCORER
        )
    else:
