...
def batched_generate_vllm(items: List[Dict[str, Any]], llm: LLM, tokenizer, *, dataset_name: str, model_name: Optional[str] = None, n: int = 1, 
...
def evaluate_vllm(dataset, llm: LLM, tokenizer, *, ds_name: str, model_name: Optional[str] = None, limit: Optional[int] = None, n: int = 1, temperature: float = 0.2, top_p: float = 0.9, seed: int = 123, max_tokens: int = 512, 
    ...
    for start, batch in _chunk(list(range(N)), batch_size):
        ...
        if ds_name == "mmlu":
            ...
            gens_batch = batched_generate_vllm(items, llm, tokenizer, dataset_name=ds_name, model_name=model_name, n=n,
            ...
            gens_batch: List[List[List[str]]] = batched_generate_vllm(
                items, llm, tokenizer,
                dataset_name=ds_name, model_name=model_name, n=n,
...
#         masked = probs[i][token_masks[i]]  # (S, C) or empty
#         if masked.numel() == 0:
#             all_scores.append([])
#         else: # take P(label=1)
...
def score_candidates_with_prm(prm_model, prm_tokenizer, question: str, candidates: List[str], rw_token: str = "<RW>", ds_name: str = "", choices: Optional[List[str]] = None, model: Optional[Any]= None) -> Tuple[List[List[float]], List[float]]:
    if prm_tokenizer.pad_token_id is None:
        prm_tokenizer.pad_token = prm_tokenizer.eos_token

    conv_strs = [build_prm_prompt(prm_tokenizer, question, split_steps(txt), rw_token=rw_token, ds_name=ds_name, choices=choices) for txt in candidates]
...
def pick_best_by_prm(prm_model,prm_tokenizer,question: str, candidates: List[str], agg: str = "mean", rw_token: str = "<RW>", ds_name: str = "", choices: Optional[List[str]] = None,) -> Tuple[int, List[List[float]], List[float]]:
    step_scores = score_candidates_with_prm(prm_model=prm_model, prm_tokenizer=prm_tokenizer,
            question=question, candidates=candidates, rw_token=rw_token, ds_name=ds_name, choices=choices,)
...
def evaluate_dataset_bon_vllm(dataset, llm: LLM, policy_tokenizer, prm_model, prm_tokenizer, *, ds_name: str, model_name: Optional[str] = None, limit: Optional[int] = None, n: int = 8, temperature: float = 0.6, top_p: float = 0.95, seed: int = 123, 
    ...
    for start, batch_idx in _chunk(all_indices[:N], batch_size):
        ...
        gens_batch = batched_generate_vllm(items, llm, policy_tokenizer, dataset_name=ds_name, model_name=model_name, n=n, temperature=temperature, top_p=top_p, max_tokens=max_tokens, seed=seed,)
        ...
        if ds_name == "mmlu":
            ...
            for j, i_ex in enumerate(batch_idx):
                ...
                best_idx, step_scores, agg_scores = pick_best_by_prm(prm_model, prm_tokenizer, q, gens, agg=agg, rw_token="<RW>", ds_name="mmlu", choices=choices,)
            ...
            for j, i_ex in enumerate(batch_idx):
                ...
                best_idx, step_scores, agg_scores = pick_best_by_prm(prm_model, prm_tokenizer, q, gens, agg=agg, rw_token="<RW>")
...
if __name__ == "__main__":
    ...
    ap.add_argument('--dataset', type=str, default='gsm8k', choices=['gsm8k', 'math', 'mmlu', 'ob', "omni", 'aime'])
    ...
    ap.add_argument('--reward_type', type=str, default='mi_margin', choices=['mi_loo', 'mi_shapley', 'mi_margin'])
    ap.add_argument('--norm_type', type=str, default='logquant', choices=['zsig', 'logquant', 'exp'])
    ...
    if need_prm:
        # path = f"/home/leena/rs_prm/checkpoints/qw3_4b/{args.reward_type}/{args.norm_type}_local_pos/final_model"
        path = "/home/leena/rs_prm/checkpoints/qw3_4b/qval/final_model"
        ...
        print(f"Load {args.norm_type}, {args.reward_type} PRM Model!")
    
    # Evaluation
    incorr_path = f"/home/leena/rs_prm/analysis/qw3_4b/{args.dataset}_{args.infer_type}.json"
    if args.infer_type == 'single':
        print(f"Evaluating {args.dataset} with single generation...", flush=True)
        acc, logs, incorrect = evaluate_vllm(datas, policy, tokenizer, ds_name=args.dataset, model_name=model_name,
        ...
        acc, logs, incorrect = evaluate_dataset_bon_vllm(datas, policy, tokenizer, prm_model, prm_tokenizer, ds_name=args.dataset, model_name=model_name,
