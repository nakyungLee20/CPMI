...
def score_candidates_with_prm(prm_model, prm_tokenizer, question: str, candidates: List[str], rw_token: str = "<RW>", ds_name: str = "", model_type: str = "",
    choices: Optional[List[str]] = None, model: Optional[Any]= None, prm_micro_bs: int = 4):
    if prm_tokenizer.pad_token_id is None:
        prm_tokenizer.pad_token = prm_tokenizer.eos_token

    conv_strs = [build_prm_prompt(prm_tokenizer, question, split_steps(txt), rw_token=rw_token, ds_name=ds_name, model_type=model_type, choices=choices)
                 for txt in candidates]

    enc = prm_tokenizer(conv_strs, return_tensors="pt", padding=True, truncation=True, max_length=prm_tokenizer.model_max_length)

    dev = (prm_model.get_input_embeddings().weight.device if hasattr(prm_model, "get_input_embeddings") else next(prm_model.parameters()).device)
    ...
    import torch
    scores_chunks, lengths_all = [], []
    start_bs = max(1, int(prm_micro_bs))
    bs = start_bs

    i = 0
    with torch.inference_mode():
        while i < input_ids.size(0):
            j = min(i + bs, input_ids.size(0))
            try:
                ids_s = input_ids[i:j]
                am_s  = attention_mask[i:j]
                rw_s  = rw_positions[i:j]
                # keep dtype slim
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    scores_flat_s, lengths_s = prm_model.scores_at_rw(input_ids=ids_s, attention_mask=am_s, rw_positions=rw_s)
                scores_chunks.append(scores_flat_s.detach().cpu())
                lengths_all.extend(int(x) for x in lengths_s)
                del ids_s, am_s, rw_s, scores_flat_s, lengths_s
                i = j  # advance
            except torch.OutOfMemoryError:
                torch.cuda.empty_cache()
                if bs == 1:
                    raise  # nothing else we can do
                bs = max(1, bs // 2)  # back off and retry with smaller micro-batch

    scores_flat = torch.cat(scores_chunks, dim=0)

    # reconstruct per-candidate step scores
    all_step_scores = []
    offset = 0
    for idx, L in enumerate(lengths_all):
        Li = int(L)
        step_scores_i = scores_flat[offset:offset + Li].tolist()
        offset += Li
        if len(split_steps(candidates[idx])) != Li:
            print(f"[WARN] steps({len(split_steps(candidates[idx]))}) != RW tokens({Li}) for sample {idx}", flush=True)
...
def pick_best_by_prm(prm_model,prm_tokenizer,question: str, candidates: List[str], agg: str = "mean", rw_token: str = "<RW>", ds_name: str = "", model_type: str ="", choices: Optional[List[str]] = None,) -> Tuple[int, List[List[float]], List[float]]:
    step_scores = score_candidates_with_prm(prm_model=prm_model, prm_tokenizer=prm_tokenizer, question=question, candidates=candidates, 
                                            rw_token=rw_token, ds_name=ds_name, model_type=model_type, choices=choices, prm_micro_bs=8,)
...
if __name__ == "__main__":
    ...
    ap.add_argument('--dataset', type=str, default='math', choices=['gsm8k', 'math', 'mmlu', 'ob', "omni", 'aime'])
    ...
    ap.add_argument('--max_new_tokens', type=int, default=2400)
    ...
    incorr_dir = f"/home/leena/rs_prm/analysis/hard_80k/bon{args.n}"
