...
def eval_processbench(prm_dir: str, configs: List[str], out_dir: str, batch_size: int = 32, threshold: Optional[float] = None, use_prob: bool = False, rw_token: Optional[str] = "<RW>",
    ...
    need_dev_global = (threshold is None) or (invert_score is None) or (calibration_mode not in (None, "", "none", "NONE"))
    
    # ---------- tune on dev (only when needed) ----------
    if need_dev_global:
        try:
            ds_dev_global = load_dataset("Qwen/ProcessBench", split="gsm8k")
            dev_scores_g, dev_labels_g = _collect_dev_chunk_scores(prm, tokenizer, ds_dev_global, budget_examples=dev_budget_examples, batch_size=batch_size, rw_token=getattr(prm, "rw_token", rw_token) or "<RW>",)

            scores_for_tuning_g = dev_scores_g
            cal_mode = (calibration_mode or "").lower().strip()
            if cal_mode and cal_mode != "none":
                if cal_mode == "platt":
                    global_calibrator = fit_platt_scaler(dev_scores_g, dev_labels_g)
                    if global_calibrator:
                        scores_for_tuning_g = apply_platt_scaler(dev_scores_g, global_calibrator)
                        print(f"[TUNE][GLOBAL] Platt calibration scale={global_calibrator['scale']:.6f}, bias={global_calibrator['bias']:.6f}")
                    else:
                        print("[TUNE][GLOBAL] Platt calibration skipped (insufficient signal).")
                else:
                    raise ValueError(f"Unsupported calibration_mode='{calibration_mode}'")

            tune_g = choose_direction_and_threshold(scores_for_tuning_g, dev_labels_g, try_patiences=(1,2,3), use_prob=use_prob, force_invert=None if invert_score is None else bool(invert_score))
            if global_invert is None:
                global_invert = tune_g["invert"]
            if global_threshold is None:
                global_threshold = tune_g["threshold"]
            if global_patience is None or global_patience == 0:
                global_patience = tune_g["patience"]
            print(f"[TUNE][GLOBAL] invert={global_invert}, patience={global_patience}, threshold={global_threshold:.4f} | "
                  f"dev_F1={tune_g['f1']:.1f}, err_acc={tune_g['err_acc']:.1f}, cor_acc={tune_g['cor_acc']:.1f}, AUC={tune_g['auc']:.3f}, AUC(-)={tune_g['auc_inv']:.3f}")
        except Exception as e:
            print("[WARN] GLOBAL DEV tuning failed, fallback to defaults:", repr(e))
            global_calibrator = None
            if global_invert is None:   global_invert = False
            if global_threshold is None: global_threshold = 0.0 if not use_prob else 0.5
            global_patience = patience or 1

    # ---------- Per-split evaluation (with optional per-split tuning) ----------
    for cfg in configs:
        ds_full = load_dataset("Qwen/ProcessBench", split=cfg)
        N_full = len(ds_full)
        # Defaults: use global tuned params
        split_invert   = global_invert
        split_thr      = global_threshold
        split_patience = global_patience
        split_calib: Optional[Dict[str, float]] = global_calibrator

        # dev/test 분할
        if tune_on_split and split_dev_size > 0 and N_full > 1:
            m = min(max(1, split_dev_size), max(1, N_full // 2))  # 최대 절반까지만 dev로
            rng = np.random.RandomState(0)
            perm = rng.permutation(N_full).tolist()
            dev_idx  = perm[:m]
            test_idx = perm[m:]
            ds_dev_split  = ds_full.select(dev_idx)
            ds_test_split = ds_full.select(test_idx)
            print(f"[TUNE][{cfg}] per-split dev={len(ds_dev_split)} test={len(ds_test_split)}")

            # 1) dev 스코어 수집
            dev_scores_s, dev_labels_s = _collect_dev_chunk_scores(prm, tokenizer, ds_dev_split, budget_examples=len(ds_dev_split), batch_size=batch_size, rw_token=getattr(prm, "rw_token", rw_token) or "<RW>",)
            # 2) per-split calibration 적합
            split_calib = None
            cal_mode = (calibration_mode or "").lower().strip()
            scores_for_tuning_s = dev_scores_s
            if cal_mode and cal_mode != "none":
                if cal_mode == "platt":
                    split_calib = fit_platt_scaler(dev_scores_s, dev_labels_s)
                    if split_calib:
                        scores_for_tuning_s = apply_platt_scaler(dev_scores_s, split_calib)
                        print(f"[TUNE][{cfg}] Platt calibration scale={split_calib['scale']:.6f}, bias={split_calib['bias']:.6f}")
                    else:
                        print(f"[TUNE][{cfg}] Platt calibration skipped (insufficient signal).")
            # 3) per-split threshold / invert / patience 탐색
            tune_s = choose_direction_and_threshold(scores_for_tuning_s, dev_labels_s, try_patiences=(1,2,3), use_prob=use_prob, force_invert=None if invert_score is None else bool(invert_score))
            split_invert   = tune_s["invert"] if invert_score is None else bool(invert_score)
            split_thr      = tune_s["threshold"] if threshold is None else float(threshold)
            split_patience = tune_s["patience"] if (patience in (None, 0)) else int(patience)
            print(f"[TUNE][{cfg}] invert={split_invert}, patience={split_patience}, threshold={split_thr:.4f} | "
                  f"dev_F1={tune_s['f1']:.1f}, err_acc={tune_s['err_acc']:.1f}, cor_acc={tune_s['cor_acc']:.1f},  AUC={tune_s['auc']:.3f}, AUC(-)={tune_s['auc_inv']:.3f}")
            ds_eval = ds_test_split
        else:
            ds_eval = ds_full
        ...
        for i in tqdm(range(0, N, batch_size), desc=f"Processing {cfg}", dynamic_ncols=True):
            batch = ds_eval.select(range(i, min(i + batch_size, N)))
            problems = [ex["problem"] for ex in batch]
            steps_list = [list(ex["steps"]) for ex in batch]
            ...
            elif global_calibrator:
                step_scores_list = apply_platt_scaler(step_scores_list, global_calibrator)
            ...
            for step_scores in step_scores_list:
                pred = predict_first_error(step_scores, threshold=split_thr, use_prob=use_prob, invert_score=split_invert, patience=split_patience)
        ...
        if debug:
            ...
            loc = error_localization_stats(predictions, list(ds_eval["label"]), out_dir=split_out, split_name=cfg)
...
def main():
    ...
    ap.add_argument("--tune_on_split", action="store_false", help="Tune invert/threshold/patience(+calibration) on a small dev slice drawn from each split itself.")
    ap.add_argument("--split_dev_size", type=int, default=250, help="Number of examples to use for per-split dev tuning (sampled without replacement).")
    ap.add_argument("--dev_budget_examples", type=int, default=380)
    ap.add_argument("--out_dir", type=str, default=None)
    ap.add_argument("--analysis_root", type=str, default="/home/leena/rs_prm/analysis/processbench")
    args = ap.parse_args()
    ...
    eval_processbench(
        ...
        # invert_score=(args.invert_score if args.invert_score else None),
        invert_score=False,
