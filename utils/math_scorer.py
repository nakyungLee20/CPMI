...
class MathScorer:
    ...
    def _interval_to_sympy(self, s: str):
        ...
        def _parse_end(u: str):
            ...
            # --- Clean first
            cleaned = self._latex_cleanup(u)

            # --- Guards (동일하게 적용)
            if (len(cleaned) > 300 or
                re.search(r'\*\*\s*-?\d{4,}', cleaned) or
                re.search(r'(?:\*\*\s*){2,}|\^{2,}', cleaned) or
                re.search(r'[²³⁴⁵⁶⁷⁸⁹]', cleaned) or
                re.search(r'\{[^{}]*\}\s*(\^|\*\*)', cleaned) or
                re.search(r'FiniteSet\s*\([^)]*\)\s*(\^|\*\*)', cleaned)):
                return None

            # --- Light LaTeX parse (가드 후에 시도)
            try:
                if parse_latex is not None and re.search(r"\\[A-Za-z]+", cleaned):
                    return parse_latex(cleaned)
            except Exception:
                pass

            # --- Safe ascii parse (no evaluation)
            try:
                return parse_expr(cleaned, transformations=TRANSFORMS, evaluate=False)
            except Exception:
                return None
    ...
    # def _looks_heavy_expr(self, e: sp.Expr) -> bool:
    #     """심볼 트리를 훑어서 거대 거듭제곱/노드 폭발 등 위험 패턴이면 True."""
    #     try:
    #         from sympy import Pow, Integer, Float, factorial, gamma
    #         nodes = 0
    #         for node in sp.preorder_traversal(e):
    #             nodes += 1
    #             if nodes > 2000:              # 노드 수 과다
    #                 return True
    ...
    #                     try:
    #                         if abs(int(exp)) >= 200:   # 임계값은 경험적으로 조정
    #                             return True
    #                     except Exception:
    #                         return True
    #                 # 큰 정수/부동소수 ** 중간 지수도 위험
    #                 base = node.base
    #                 if isinstance(base, (sp.Integer, sp.Float)):
    #                     if len(str(base)) >= 200 and exp.is_integer and exp.is_number:
    #                         try:
    #                             if abs(int(exp)) >= 20:
    #                                 return True
    #                         except Exception:
    #                             return True
    #             # 팩토리얼/감마 등도 평가 시 폭발
    #             if getattr(node, 'func', None) in {sp.factorial, sp.gamma}:
    #                 return True
    #         return False
    #     except Exception:
    #         # 분석 실패 시 보수적으로 위험 취급
    ...
    def _looks_heavy_expr(self, e: sp.Expr) -> bool:
        try:
            from sympy import Pow, Integer, Rational, Float, Symbol, factorial, gamma
            nodes = 0
            for node in sp.preorder_traversal(e):
                nodes += 1
                if nodes > 2000:
                ...
                if getattr(node, 'func', None) in {sp.factorial, sp.gamma}:
                    return True
                # 거듭제곱 패턴만 가볍게 검사 (절대 evalf/str/int 호출 X)
                if isinstance(node, Pow):
                    exp = node.exp
                    base = node.base
                    # 지수: Integer만 안전 비교. 그 외(복합·Float·Symbol)는 위험 처리
                    if isinstance(exp, Integer):
                        # 2**200 같은 큰 지수 차단
                        if exp.bit_length() >= 8 or abs(int(exp)) >= 200:
                            return True
                    else:
                        # 지수가 정수가 아니면 평가 위험
                        return True
                    # 밑: 아주 큰 정수 자체도 위험 (문자열 변환 금지)
                    if isinstance(base, Integer):
                        # 비트 길이로 크기 근사 (대충 2**1024 이상은 차단)
                        if base.bit_length() >= 1024:
                            return True
                    elif isinstance(base, Float):
                        # Float 밑에 큰 정수 지수도 위험
                        return True
                    else:
                        # 심볼/합성식 밑은 보수적으로 위험
                        return True
    ...
    def _sympy_parse(self, expr: str):
        """Safe parser: never lets SymPy 'evaluate' huge powers; returns a SymPy Expr/Set or None."""
        if expr is None:
            return None
        raw = str(expr).strip()
        if not raw:
        ...
        # ---- Hard guards against heavy expressions ----
        TOO_LONG = 300  # tune if needed (200~400)
        BIG_EXP_RE     = re.compile(r'\*\*\s*-?\d{4,}')        # e.g., **10000
        POW_TOWER_RE   = re.compile(r'(?:\*\*\s*){2,}|\^{2,}') # '** **' chains or '^^'
        UNICODE_POW_RE = re.compile(r'[²³⁴⁵⁶⁷⁸⁹]')            # unicode superscripts

        # Try interval unions first
        ...
        # Clean and latex→ascii normalization
        cleaned = self._latex_cleanup(raw)

        # Guard: set^power or FiniteSet(...)**k → reject symbolic
        # SET_POW_RE       = re.compile(r'^\s*\{[^{}]+\}\s*(\^|\*\*)\s*[-+]?\d+\s*$')
        # FINITESET_POW_RE = re.compile(r'^\s*FiniteSet\([^)]*\)\s*(\^|\*\*)\s*[-+]?\d+\s*$')
        # if SET_POW_RE.search(cleaned) or FINITESET_POW_RE.search(cleaned):
        #     return None
        
        SET_ANY_POW_RE       = re.compile(r'\{[^{}]*\}\s*(\^|\*\*)')
        FINITESET_ANY_POW_RE = re.compile(r'FiniteSet\s*\([^)]*\)\s*(\^|\*\*)')
        if SET_ANY_POW_RE.search(cleaned) or FINITESET_ANY_POW_RE.search(cleaned):
            return None 

        # Global hard guards
        if (len(cleaned) > TOO_LONG or
            BIG_EXP_RE.search(cleaned) or
            POW_TOWER_RE.search(cleaned) or
            UNICODE_POW_RE.search(cleaned)):
            return None

        # Light LaTeX parse first
        try:
            if parse_latex is not None and re.search(r"\\[A-Za-z]+", cleaned):
                return parse_latex(cleaned)
        except Exception:
    ...
    #     """숫자/컨테이너 우선, 심볼릭은 무거운 패턴이면 즉시 우회."""
    #     if sp is None:
    #         return False

    #     # 0) 숫자 비교 (이미 구현돼 있는 가장 빠른 경로)
    #     try:
    #         na = self._parse_numeric(a_str)
    #         nb = self._parse_numeric(b_str)
    ...
    #             return (math.isfinite(va) and math.isfinite(vb)
    #                     and abs(va - vb) <= self.atol)
    #     except Exception:
    #         pass

    #     # 1) 안전 파싱 (evaluate=False) — 기존 _sympy_parse 사용
    #     a = self._sympy_parse(a_str)
    #     b = self._sympy_parse(b_str)
    #     if a is None or b is None:
    #         return False

    #     # 2) 파싱 이후에도 위험 트리면 여기서 즉시 False
    #     if self._looks_heavy_expr(a) or self._looks_heavy_expr(b):
    #         return False

    #     # 3) 집합/인터벌은 구조적 비교만 (simplify 호출 제거)
    #     try:
    #         if isinstance(a, (sp.Set, sp.Interval)) or isinstance(b, (sp.Set, sp.Interval)):
    #             # 가능한 한 직접 비교만 시도하고, 실패 시 False
    #             try:
    #                 return a == b
    #             except Exception:
    #                 return False
    ...
    #     # 4) 차이를 만들고 'is_zero'를 묻는 것도 내부에서 evalf/assumptions를 태울 수 있어 제외
    #     #    대신, 아주 제한된 샘플링만 수행 (지수/노드가 가벼운 경우에만)
    #     try:
    #         vars = sorted(list((a.free_symbols | b.free_symbols)), key=lambda s: s.name)  # type: ignore[attr-defined]
    #         if not vars:
    #             return False

    #         # 샘플링도 가볍게: 작은 정수만, 실패 즉시 중단
    #         for val in (-2, -1, 0, 1, 2, 3):
    #             subs = {v: val for v in vars}
    #             # evalf가 또 pow를 태울 수 있으므로 숫자로 강제 변환 전에 추가 방어
    #             av = a.subs(subs)
    #             bv = b.subs(subs)

    #             # 대입 후에도 여전히 위험하다면 바로 실패 처리
    #             if self._looks_heavy_expr(av) or self._looks_heavy_expr(bv):
    #                 return False

    #             # 가능하면 정수/유리/유한 소수로만 비교
    #             if av.is_number and bv.is_number:
    #                 try:
    #                     # Rational이면 float로 변환(저정밀)하여 비교
    #                     af = float(av.evalf(30))
    #                     bf = float(bv.evalf(30))
    #                 except Exception:
    #                     return False
    ...
    def _sympy_equal(self, a_str: str, b_str: str) -> bool:
        ...
        # 0) 빠른 수치 경로 (Fraction/float parse)
        try:
            na = self._parse_numeric(a_str)
            nb = self._parse_numeric(b_str)
            if (na is not None) and (nb is not None):
        ...
        # 4) 완전한 수치형일 때만 비교 (Rational/Integer/Float 간단 케이스)
        #    evalf 없이 비교 가능한 경우만 허용
        if (a.is_Number and b.is_Number):
            # Integer/Rational이면 그 자체로 비교 가능
            if (a.is_Integer or a.is_Rational) and (b.is_Integer or b.is_Rational):
                return a == b
            # Float 둘 다일 때만 비교 (복합 Pow가 아닌 진짜 Float)
            if isinstance(a, sp.Float) and isinstance(b, sp.Float):
                return abs(float(a) - float(b)) <= self.atol
            return False

        # 5) 변수 포함/복합식 동치 검증은 스레드 모드에서 비활성화 (위험 차단)
        return False
