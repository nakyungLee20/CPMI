...
class MathScorer:
    ...
    def _looks_heavy_expr(self, e: sp.Expr) -> bool:
        try:
            from sympy import Pow, Integer, Rational, Float, factorial, gamma
            nodes = 0
            for node in sp.preorder_traversal(e):
                nodes += 1
                if nodes > 2000:          # 구조가 너무 크면 위험
                    return True
                # factorial / gamma 는 바로 차단
                if getattr(node, 'func', None) in {factorial, gamma}:
                    return True
                # 거듭제곱 패턴 검사
                if isinstance(node, Pow):
                    exp = node.exp
                    ...
                    # (1) 지수: Integer
                    if isinstance(exp, Integer):
                        # 2**200, 10**1000 이런 건 막기
                        if exp.bit_length() >= 8 or abs(int(exp)) >= 200:
                            return True
                    # (2) 지수: Rational (예: 1/2, 3/2, -1/3 등)
                    elif isinstance(exp, Rational):
                        # 분자/분모가 너무 크면 위험한 유리 지수로 간주
                        if abs(exp.p) > 10 or abs(exp.q) > 10:
                            return True
                        # 예: 2^(1/2), 3^(3/2) 같은 건 허용
                    # (3) 그 외 지수 (심볼, 복합식 등) → 보수적으로 차단
                    else:
                        return True
                    # 밑이 아주 큰 정수면 역시 위험
                    if isinstance(base, Integer):
                        if base.bit_length() >= 1024:   # 대략 2**1024 이상
                            return True
            # 위 조건들에 안 걸리면 '그나마 안전'
            return False
        except Exception:
            # 분석 도중 에러 나면 보수적으로 위험 취급
            return True
    ...
    def _sympy_equal(self, a_str: str, b_str: str) -> bool:
        ...
        # 0) 빠른 수치 경로 (Fraction/float parse) - SymPy 안 쓰는 경로
        try:
            na = self._parse_numeric(a_str)
            nb = self._parse_numeric(b_str)
            if (na is not None) and (nb is not None):
        ...
        # 4) 순수 수치형 표현만 SymPy numeric 비교 허용
        #    예) sqrt(2)/2 vs 1/sqrt(2), pi/4 vs 0.25*pi 등
        if a.is_number and b.is_number and not (a.free_symbols or b.free_symbols):
            # (4-1) Integer / Rational 은 exact 비교 (가장 싸고 안전)
            if (a.is_Integer or a.is_Rational) and (b.is_Integer or b.is_Rational):
                try:
                    return a == b
                except Exception:
            ...
            # (4-2) 그 외 numeric (sqrt, pi 포함)은 evalf로 근사 비교
            try:
                av = complex(a.evalf(40))
                bv = complex(b.evalf(40))
            except Exception:
                return False

            # 유한 수만 허용
            if not (math.isfinite(av.real) and math.isfinite(av.imag)
                    and math.isfinite(bv.real) and math.isfinite(bv.imag)):
                return False

            diff = abs(av - bv)
            scale = max(abs(av), abs(bv), 1.0)
            # 절대/상대 오차를 동시에 잡는 식으로 비교
            return diff <= max(self.atol, 1e-10 * scale)

        # 5) 변수 포함/복합식 동치 검증은 여전히 비활성화 (안전 우선)
        return False
