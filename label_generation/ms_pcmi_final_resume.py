...
from cpmi_reward_final import CPMIEnsembleReward
...
def main():
    ...
    mi = CPMIEnsembleReward(model=model, tokenizer=tokenizer)
    ...
    # --- outputs per shard ---
    out_dir = "/home/leena/rs_prm/datasets/hard_80k/cpmi_final"
    os.makedirs(out_dir, exist_ok=True)
    shard_tag = f"s{args.shard_index}of{args.num_shards}"
    prefix = f"ms_cpmi_final_{shard_tag}"
    ...
    header = f"MathShepherd/cpmi_final :: model={model_name} :: shard={shard_tag} :: rows={len(ds_resume)} :: from={already}"
