...
from cpmi_simple_support import CPMISimpleReward
...
def parse_args():
    ...
    ap.add_argument("--num_shards", type=int, default=2)         # <<< default 3-way split
    ap.add_argument("--shard_index", type=int, default=0)        # 0,1,2
    ap.add_argument("--mnum", type=int, default=4)              # M number for ablation
    ap.add_argument("--nsam", type=int, default=8)              # number of sampled negatives
    return ap.parse_args()

def main():
    args = parse_args()
    model_name = "Qwen/Qwen3-8B-base"  # "Qwen/Qwen2.5-Math-7B-Instruct" "mistralai/Mathstral-7b-v0.1" "Qwen/Qwen3-8B-base"
    model = AutoModelForCausalLM.from_pretrained(model_name, device_map="auto", torch_dtype="auto", trust_remote_code=True)
    tokenizer = AutoTokenizer.from_pretrained(model_name, use_fast=False)
    mi = CPMISimpleReward(model=model, tokenizer=tokenizer)
    ...
    # --- outputs per shard ---
    out_dir = "/home/leena/rs_prm/datasets/hard_80k/qw8b/ablation"
    os.makedirs(out_dir, exist_ok=True)
    shard_tag = f"s{args.shard_index}of{args.num_shards}"
    prefix = f"ms_cpmi_m{args.mnum}_{shard_tag}"
    print(f"[INFO] prefix: {prefix} with nsam={args.nsam} mnum={args.mnum}")
    ...
    with open(output_file, "w", encoding="utf-8", buffering=1) as f:
        ...
        for i, entry in enumerate(mi.mi_labelling(ds=ds_resume, ds_task_tag="task", n_sample_negs=args.nsam, max_negatives=args.mnum), start=already):
    ...
    header = f"MathShepherd/cpmi_simple :: model={model_name} :: shard={shard_tag} :: rows={len(ds_resume)} :: from={already}"
