...
from cpmi_contrastive_reward import CPMIContrastiveReward
...
def parse_args():
    ...
    ap.add_argument("--num_shards", type=int, default=8)         # <<< default 3-way split
...
def main():
    ...
    mi = CPMIContrastiveReward(model=model, tokenizer=tokenizer)
    ...
    # --- outputs per shard ---
    out_dir = "/home/leena/rs_prm/datasets/hard_80k/cpmi_contrast"
    os.makedirs(out_dir, exist_ok=True)
    shard_tag = f"s{args.shard_index}of{args.num_shards}"
    prefix = f"ms_cpmi_contrast_time_{shard_tag}"
    ...
    header = f"MathShepherd/cpmi_contrast :: model={model_name} :: shard={shard_tag} :: rows={len(ds_resume)} :: from={already}"
