...
from cpmi_prompt_div import CPMIPromptReward
...
def main():
    ...
    mi = CPMIPromptReward(model=model, tokenizer=tokenizer)
    ...
    # --- outputs per shard ---
    out_dir = "/home/leena/rs_prm/datasets/hard_80k/cpmi_prompt"
    os.makedirs(out_dir, exist_ok=True)
    shard_tag = f"s{args.shard_index}of{args.num_shards}"
    prefix = f"ms_cpmi_prompt_qw3_4b_{shard_tag}"
    ...
    header = f"MathShepherd/cpmi_prompt :: model={model_name} :: shard={shard_tag} :: rows={len(ds_resume)} :: from={already}"
