...
PROMPT_TEMPLATES = [
    ...
    (
        "Work through the solution concisely, step by step. Conclude with exactly one line:\n"
        "The answer is: <final answer>\n\n"
        "Problem: {q}\nSolution (step-by-step):\n"
    ),
    (
        "Think carefully and keep notation consistent. Provide short, numbered steps.\n"
...
class CPMIEnsembleReward:
    ...
    def __init__(self, model, tokenizer):
        ...
        self.neg_clip_mode = "none"   # "none" | "poscap"
        self.neg_clip_tau  = 1.5  
    ...
    def _robust_normalize_01(self, xs: List[float]) -> List[float]:
        if not xs: return []
        import numpy as np
    ...
    def _lp_mean_over_targetset_batch(self, prompts: List[str], targets: List[str], batch_size: int = 12, normalize: bool = True) -> List[float]:
        if not targets:
            return [0.0] * len(prompts)
        P, T = [], []
        for t in targets:
            P.extend(prompts)
            T.extend([t] * len(prompts))
        if normalize:
            vals = self._logprob_mean_batch(P, T, batch_size)
        else:
            pairs = self._logprob_total_batch(P, T, batch_size)
            vals = [s for (s, L) in pairs]
        Lp = len(prompts)
        out = [0.0] * Lp
        for i in range(Lp):
            span = vals[i::Lp]  # i, i+Lp, i+2Lp, ...
            out[i] = sum(span) / float(len(span))
        return out
    ...
    @torch.no_grad()
    def _sample_model_negatives_mixed(self, question: str, steps: List[str], gold: str, n: int = 6, cond_ratio: float = 0.25, 
        max_new_tokens: int = 24, temperature: float = 0.9, top_p: float = 0.9, log_ctx: Optional[Dict[str, Any]] = None, template_id_plain: int = 0, ) -> Tuple[List[str], int]:
        t0 = time.perf_counter()
        n_cond  = max(0, int(round(n * cond_ratio)))
        n_plain = max(0, n - n_cond)
        g = self._norm_ans_text(gold)

        def _gen(prompt: str, num: int, temp: float) -> Tuple[List[str], int]:
            if num <= 0: return [], 0
            enc = self.tokenizer(prompt, return_tensors="pt").to(self.device)
            input_len = enc["input_ids"].shape[1]
            out = self.model.generate(**enc, do_sample=True, temperature=temp, top_p=top_p,
                num_return_sequences=num, max_new_tokens=max_new_tokens,
            ...
            for t in texts:
                ...
                if ans and ans != g and len(ans) <= 32:
                    cands.append(self._wrap_ans(ans))
            return cands, total_new

        total_new = 0
        pool: List[str] = []
        # 3-1) Q-only (Jeffreys 근사에 대응)
        prompt_plain = self.build_prompt(question, template_id_plain) + "\nThe answer is: "
        c1, new1 = _gen(prompt_plain, n_plain, temperature)
        pool += c1; total_new += new1
        # 3-2) step-조건 (최근 1~2개 step만, 온도 살짝↓)
        if steps and n_cond > 0:
            few = steps[-2:] if len(steps) >= 2 else steps
            prefix = self.build_prompt(question, template_id_plain) + "\n" + "".join(s.rstrip().rstrip("\n") + "\n\n" for s in few)
            prompt_cond = prefix + "The answer is: "
            c2, new2 = _gen(prompt_cond, n_cond, max(0.7, temperature - 0.1))
            pool += c2; total_new += new2
        # 유효성 + 중복 제거
        uniq, seen = [], set([self._wrap_ans(g)])
        for a in pool:
            if a not in seen and self._is_valid_wrapped(a):
                seen.add(a); uniq.append(a)

        try:
            self.prof.log(tag="neg:sample.mixed", wall_s=time.perf_counter()-t0, gen_tokens=total_new, num_samples=len(uniq), **(log_ctx or {}))
        except Exception:
            pass
        return uniq, total_new
    
    def _is_valid_wrapped(self, a_wrapped: str) -> bool:
        ans = self._extract_ans_from_text(a_wrapped)
        return bool(ans) and (len(ans) <= 32)

    def _build_wrong_answer_candidates(self, question: str, steps: List[str], gold_answer: str, max_candidates: int = 8, n_sample: int = 4, log_ctx: Optional[Dict[str, Any]] = None) -> Tuple[List[str], int]:
        gold_answer = self._norm_ans_text(gold_answer)
        pool = []
        gen_tok = 0
        # 1) model-sampled (logged with gen_tokens above)
        try: 
            cands, t_new = self._sample_model_negatives_mixed(question, steps, gold_answer, n=n_sample, cond_ratio=0.25, log_ctx=log_ctx)
            pool += cands; gen_tok += int(t_new)
        ...
        if len(uniq) < max_candidates:
            more = self._heuristic_negatives(gold_answer, max_h=max_candidates*2)
            for a in more:
                if a not in seen and self._is_valid_wrapped(a):
                    seen.add(a); uniq.append(a)
                    if len(uniq) >= max_candidates: break
        return uniq[:max_candidates], gen_tok

    def _clip_neg_delta(self, d: float) -> float:
        # mode="poscap": 양수인 경우만 tau로 캡핑, 음수는 그대로(도움되는 신호라 보존)
        if getattr(self, "neg_clip_mode", "poscap") != "poscap":
            return d
        return min(d, self.neg_clip_tau) if d > 0.0 else d
    ...
    def compute_step_contrastive_cmi_ensemble_batch(self, question: str, steps: List[str], gold_target: str, neg_targets: List[str],
        ...
        # negatives
        if neg_targets:
            n_prev = self._lp_mean_over_targetset_batch(all_prev, neg_targets, batch_size, normalize)
            n_with = self._lp_mean_over_targetset_batch(all_with, neg_targets, batch_size, normalize)
            cur = 0
            neg_acc = [0.0]*S
            for _ in range(K):
                for i in range(S):
                    # neg_delta = (n_with[cur+i] - n_prev[cur+i])
                    # neg_acc[i] += self._clip_neg_delta(neg_delta)
                    neg_acc[i] += (n_with[cur+i] - n_prev[cur+i])
                cur += S
            out = [(out[i]/K) - (neg_acc[i]/K) for i in range(S)]
        else:
    ...
    def compute_step_contrastive_marginal_ensemble_batch(self, question: str, steps: List[str], gold_target: str, neg_targets: List[str],
        ...
        if neg_targets:
            n_base = self._lp_mean_over_targetset_batch(all_base_unique, neg_targets, batch_size, normalize)
            n_with = self._lp_mean_over_targetset_batch(all_with_i,      neg_targets, batch_size, normalize)
            cur = 0
            neg_acc = [0.0]*S
            for e in range(K):
                base_e = n_base[e]
                for i in range(S):
                    # neg_delta = (n_with[cur+i] - base_e)
                    # neg_acc[i] += self._clip_neg_delta(neg_delta)
    ...
    def compute_step_contrastive_loo_ensemble_batch(self, question: str, steps: List[str], gold_target: str, neg_targets: List[str],
        ...
        if neg_targets:
            n_all = self._lp_mean_over_targetset_batch(all_all_ctx, neg_targets, batch_size, normalize)
            n_wo  = self._lp_mean_over_targetset_batch(all_wo_ctx,  neg_targets, batch_size, normalize)
            neg_acc = [0.0]*S
            for e in range(K):
                o = block_starts[e]
                base_all = n_all[e]
                for i in range(S):
                    # neg_delta = (base_all - n_wo[o + i])
                    # neg_acc[i] += self._clip_neg_delta(neg_delta)
                    neg_acc[i] += (base_all - n_wo[o + i])
            out = [(out[i]/K) - (neg_acc[i]/K) for i in range(S)]
        else:
            out = [v / K for v in out]
        return out, 0
        ...
        for s in steps:
            ...
            if neg_targets:
                ...
                for neg in neg_targets:
                    ...
                    # acc += self._clip_neg_delta(n_with - n_prev)
                    acc += (n_with - n_prev)
        ...
        for s in steps:
            ...
            if neg_targets:
                ...
                for neg in neg_targets:
                    ...
                    # acc += self._clip_neg_delta(n_with - n_base)
                    acc += (n_with - n_base)
                diff -= acc / float(len(neg_targets))
            out.append(diff)
        return out
        ...
        for i in range(len(steps)):
            ...
            if neg_targets:
                ...
                for neg in neg_targets:
                    ...
                    # acc += self._clip_neg_delta(n_all - n_wo)
                    acc += (n_all - n_wo)
                diff -= acc / float(len(neg_targets))
            out.append(diff)
        return out
    ...
    def mi_labelling(self, *, ds, ds_task_tag: Optional[str] = None, template_ids: Optional[List[int]] = None, batch_size: int = 12, normalize: bool = True,
        n_sample_negs: int = 6, max_negatives: int = 8) -> Iterable[Dict[str, Any]]:
        ...
        for si, rec in tqdm(enumerate(ds)):
            ...
            # negatives
            neg_targets, neg_gen_tok = self._build_wrong_answer_candidates(question, steps, gold_answer, max_candidates=max_negatives, n_sample=n_sample_negs, log_ctx={"dataset": task, "sample_idx": si})
            M = len(neg_targets)
            ...
            #     wall_cmi = time.perf_counter() - t1
            #     num_prompts_cmi = K * 2 * len(steps) * (1 + len(neg_targets))
            #     self.prof.log(tag="cont:cmi.ensemble.naive", wall_s=wall_cmi, num_prompts=num_prompts_cmi, gen_tokens=neg_gen_tok, n=K, dataset=task, sample_idx=si, peak_mem_gb=_peak_mem_gb())
            ...
            try:
                ...
                wall_marg = time.perf_counter() - t1
                num_prompts_marg = K * (len(steps) + 1) * (1 + len(neg_targets))
                self.prof.log(tag="cont:marginal.ensemble.naive", wall_s=wall_marg, num_prompts=num_prompts_marg, gen_tokens=neg_gen_tok, n=K, dataset=task, sample_idx=si, peak_mem_gb=_peak_mem_gb())
            ...
            try:
                ...
                wall_loo = time.perf_counter() - t1
                num_prompts_loo = K * (len(steps) + 1) * (1 + len(neg_targets))
                self.prof.log(tag="cont:loo.ensemble.naive", wall_s=wall_loo, num_prompts=num_prompts_loo, gen_tokens=neg_gen_tok, n=K, dataset=task, sample_idx=si, peak_mem_gb=_peak_mem_gb())

            # Normalization
            marg_norm = self._robust_normalize_01(marg_vals)
            loo_norm  = self._robust_normalize_01(loo_vals)
            # cmi_norm = self._robust_normalize_01(cmi_vals)
