...
from cpmi_reward_ver2 import CPMIEnsembleReward
...
def main():
    ...
    # --- outputs per shard ---
    out_dir = "/home/leena/rs_prm/datasets/hard_80k/cpmi_ver2"
    os.makedirs(out_dir, exist_ok=True)
    shard_tag = f"s{args.shard_index}of{args.num_shards}"
    prefix = f"ms_cpmi_ver2_{shard_tag}"
    ...
    header = f"MathShepherd/cpmi_ver2 :: model={model_name} :: shard={shard_tag} :: rows={len(ds_resume)} :: from={already}"
