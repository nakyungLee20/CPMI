...
class TrainConfig:
    ...
    # data type
    reward_key: Optional[str] = "cpmi_marg"    # ("step_reward", "base_reward", "cpmi_marg" )
...
def _patch_cfg_from_cli(cfg: TrainConfig) -> TrainConfig:
    ...
    if args.resume_from_checkpoint:  cfg.resume_from_checkpoint = args.resume_from_checkpoint
    return cfg
...
def main(cfg: TrainConfig):
    ...
    output_dir = f"/home/leena/rs_prm/checkpoints/hard_80k/ablations/{cfg.exp_name}_{suffix}"
    os.makedirs(output_dir, exist_ok=True)
    ...
    print("Model Structure:", model, flush=True)
    ...
    args = TrainingArguments(
        ...
        report_to=["wandb"],
        run_name = f"ms_{suffix}_{cfg.exp_name}",
    )
    ...
    # 6-1) output_dir에 기존 체크포인트 있는지 확인
    ...
    if os.path.isdir(args.output_dir):
        ...
        if last_checkpoint is not None:
            ...
            print("[RESUME] No checkpoint found in output_dir.")

    # 7) Train
    print(f"Start Training with {cfg.loss_type} loss!", flush=True)
    torch.cuda.empty_cache()

    # 우선순위: CLI로 준 --resume_from_checkpoint > 자동 탐색된 last_checkpoint
    resume_path = cfg.resume_from_checkpoint or last_checkpoint
    if resume_path is not None:
        print(f"[RESUME] Training from checkpoint: {resume_path}", flush=True)
        trainer.train(resume_from_checkpoint=resume_path)
    else:
        print("[RESUME] Fresh training (no checkpoint).", flush=True)
        trainer.train()
